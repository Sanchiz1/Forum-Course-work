...
import { deleteReportRequest, requestReports } from '../../API/reportRequests';
...
interface EnhancedTableProps {
  numSelected: number;
  onRequestSort: (event: React.MouseEvent<unknown>, property: keyof Report) => void;
...
export default function ReportsDataTable() {
  ...
  const [orderBy, setOrderBy] = React.useState<keyof Report>('Id');
  ...
  React.useEffect(() => {
    requestReports(page * rowsPerPage, rowsPerPage, userTimestamp, orderBy, order).subscribe({
      next(value) {
        setRows(value)
      },
      error(err) {
                ...
                return (
                  ...
                  >
                    ...
                    <TableCell align="left">
                      <Button size='small' onClick={() => navigator("/post/" + row.PostId)}>Visit</Button>
                      <Button size='small' color="error" onClick={() => {
                        deleteReportRequest(row.Id)
                          .subscribe({
                            next(value) {
